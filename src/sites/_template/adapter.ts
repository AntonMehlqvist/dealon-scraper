...
pacing: {
hostMaxNavRps: 1.6,
ramp: [{t:0,rps:1.0},{t:900,rps:1.3},{t:3600,rps:1.6}],
...
for(const s of sels){ const el = page.locator(s).first(); if(await el.isVisible().catch(()=>false)){ await el.click().catch(()=>{}); break; } }
},
normalizeUrl: (u) => { const url = new URL(u); url.hash=''; url.search=''; return url.toString(); },
...
image: ['meta[property="og:image"]@content', 'img[data-testid*="image" i]@src'],
},
};